...
const plugin: any = definePluginEntry({
  ...
  register(api) {
    ...
    try {
      const reg = (api as any).pluginRegistry;
      if (reg?.httpRoutes) {
        const paths = reg.httpRoutes.map((r: any) => `${r.path} (${r.auth}, pluginId=${r.pluginId})`);
    ...
    registerPluginHttpRoute({
      ...
      handler: async (req, res) => {
        ...
        try {
          ...
          catch { res.statusCode = 400; res.end(JSON.stringify({ status: "bad-request" })); return true; }
    ...
    api.on("agent_end", async (event: any) => {
      ...
      debugLog(`agent_end: ${messages.length} messages, roles: ${messages.map((m: any) => m.role).join(",")}`);
      // Only forward if this turn was A2A-triggered (check any role — wake events may be system, not user)
      const hasA2A = messages.some((m: any) =>
        m.content?.includes("[a2a]") || m.content?.includes("[A2A from") || m.content?.includes("A2A from"),
      );
      ...
      const lastReply: string = assistantMsgs[assistantMsgs.length - 1]?.content || "";
      ...
      if (!tgToken) {
        const tgCfg = (api.config?.channels?.telegram || {}) as any;
